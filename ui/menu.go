		...
		dialog.ShowForm("New Image", "Create", "Cancel", formItems, func(ok bool) {
			if ok {
				...
				if widthEntry.Validate() != nil {
					dialog.ShowError(errors.New("invalid width"), app.PixelArtWindow)
				} else {
					pixelWidth, _ = strconv.Atoi(widthEntry.Text)
				}
				if heightEntry.Validate() != nil {
					dialog.ShowError(errors.New("invalid height"), app.PixelArtWindow)
				} else {
					pixelHeight, _ = strconv.Atoi(heightEntry.Text)
				}
				app.PixelArtCanvas.NewDrawing(pixelWidth, pixelHeight)
			}
		}, app.PixelArtWindow)
	})
}
...
func SetupMenus(app *AppInit) {
	...
	app.PixelArtWindow.SetMainMenu(mainMenu)
}
